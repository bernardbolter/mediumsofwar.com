...
const Enlarge = ({ imageUrl }) => {
    ...
    useEffect(() => {
        const newBounds = {
            left: size.width > 1600
            ? -(((size.width - 1600) / 2) - 40)
            : -(((1600 - size.width) / 2) + 40),

            right: size.width >1600
            ? (((size.width -1600) / 2) - 40)
            : (((1600 - size.width) / 2) + 40),

            top: size.height > 1600
            ? -(((size.height - 1600) / 2) - 40)
            : -(((1600 - size.height) / 2) + 40),

            bottom: size.height > 1600
            ? (((size.height - 1600) / 2) - 40)
            : (((1600 - size.height) / 2) + 40)
        }
        setWar(state => ({ ...state, bounds: newBounds}))
    }, [size])
    ...
    const handleDrag = (e, position) => {
        ...
        const { x, y } = position;
        console.log(x, y)
        setWar(state => ({ ...state, controlledPosition: { x: x, y: y } }))
    }
    ...
    return (
        ...
        >
            <Draggable 
                onDrag={handleDrag}
                onStop={handleStop}
                position={war.controlledPosition}
                nodeRef={nodeRef}
                bounds={war.bounds}
            >
                <div
                ...
                >
                    <img 
                        src={require(`../images/${war.paintings[war.paintingPosition].imageSlug}.jpg`).default} 
                        draggable={false}
            ...
            <div 
                ...
                onClick={() => {
                    setWar(state => ({ ...state, viewEnlarge: false, controlledPosition: { x: 0, y: 0 }}))
                }}
            >
